// Author: Sean Davis
// Edited by Ziyue Gao, Chengeng Xiao, Feb. 16, 2016 20:57
...
void Decoder::execute(const Instruction &instruction, Registers *registers, 
                      Memory &memory)
{
...
void Decoder::leal(const Instruction &instruction, const Registers *registers)
{
  ...
  ptr = strtok(NULL, " ");
  Registers::RegName regName =
  static_cast<Registers::RegName>(registers->stringToRegNum(ptr));
...
void  Decoder::leave(Registers *registers, const Memory &memory) const
{
  ...
  Data data  = dynamic_cast <const Data&> 
               (memory[registers->get(Registers::esp)]);
  registers->set(Registers::ebp, data.get());
  registers->set(Registers::esp, 
                 registers->get(Registers::esp) + 4);
}  // leave()
