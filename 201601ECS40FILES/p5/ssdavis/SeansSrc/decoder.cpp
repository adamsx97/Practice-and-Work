...
void Decoder::leal(const Instruction &instruction, const Registers *registers)
{
  ...
  *operand2 = offset + registers->get((Registers::RegName) 
    registers->stringToRegNum(ptr));
}  // leal()
...
void Decoder::leave(Registers *registers, const Memory &memory) const
{
  ...
  registers->set(Registers::ebp, 
  (dynamic_cast <const Data&> (memory[registers->get(Registers::esp)])).get());
...
{
  dynamic_cast <Data&> (memory[*registers += -4]) = *operand1;
}  // pushl()
...
{
  registers->set(Registers::eip, 
    (dynamic_cast <const Data&> 
    (memory[registers->get(Registers::esp)])).get());  
  *registers += 4;
}  // ret()
